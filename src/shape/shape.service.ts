                  ...
                  if ((col + width) <= this.matrixWidth && (row + height) <= this.matrixHeight) {
                      ...
                      innerLoop: for (let innerRow = row; innerRow <= innerRowControl; innerRow++) {
                          for (let innerCol = col; innerCol <= innerColControl; innerCol++) {
                              ...
                              if(height === 1 && width === 1 && !this.shapesArray[row][col]){

                                shapes.endingX = shapes.startingX + 1;
                                shapes.endingY = shapes.startingY + 1;
                                break innerLoop;
                              }
                              else 
                              if ((this.shapesArray[innerRow][innerCol]) ) {
                                  result = false;
                              ...
                              else if (!this.shapesArray[innerRow][innerCol]) {
